...
#if 1
// Needed as v45.2.0 does not have it yet
using Histogram3 = Acts::Experimental::Histogram<3>;
std::unique_ptr<TH3F> toRoot(const Histogram3& boostHist)
{
  ...
  std::vector<double> xEdges = Acts::Experimental::extractBinEdges(xAxis);

  // Extract bin edges from Y axis
  std::vector<double> yEdges = Acts::Experimental::extractBinEdges(yAxis);

  // Extract bin edges from Z axis
  std::vector<double> zEdges = Acts::Experimental::extractBinEdges(zAxis);
