  ...
  const ActsExamples::AlgorithmContext& ctx) const
{
  ...
  for (const auto& track : tracks) {
    ...
    // For loopers, under a certain pT, the above check might fail because the truth particle loops
    // leaving many hits in the detector. While, looper tracking is still WIP and only arcs of the full loop are
    // reconstructed, we could add a protection to reduce the impact
    // We should use the truth eta/pT probably. - PF

    if (!truthMatched && m_cfg.looperProtection) {
      auto nParticleTruthHits = particleTruthHitCount.at(majorityParticleId);
      ...
      if (hasFittedParams) {
        double theta = track.parameters()[Acts::eBoundTheta];
        track_eta = -log(tan(theta / 2));
        // track_pt = track.parameters().transverseMomentum() //track.parameters()[Acts::eBoundQOverP];
        track_pt = track.transverseMomentum(); // track.parameters()[Acts::eBoundQOverP];
      }

      if (track_pt < m_cfg.loop_maxPt && abs(track_eta) < m_cfg.loop_absEta) {

        if (nParticleTruthHits > m_cfg.loop_maxParticleHits)
          nParticleTruthHits = m_cfg.loop_maxParticleHits;
